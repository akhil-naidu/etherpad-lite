...
let prevPageBreakIndex = 0;
let bFirstTimePageBreak = true;
let allowDocPageBreak = false;
let wasPrevPageBreakFirst = false;
...
function doInsertPageBreak(insertAtLineNumber){
  ...
  insertAtLineNumber.forEach((lineNumber) => {
    var isPageBreak = documentAttributeManager.getAttributeOnLine(lineNumber, 'pageBreak');
    console.log('is page break:',isPageBreak, 'line number:', lineNumber)
    if(isPageBreak == ''){ // if its already a PageBreak item
      console.log('pagebreak is false',isPageBreak);
      ...
      console.log('pagebreak is true',isPageBreak);
...
exports.aceKeyEvent = function(hook, callstack, editorInfo, rep, documentAttributeManager, evt){
  ...
  if((evt.type == "keyup") && k != 8 ){
    ...
    y=y+height;
    console.log('total page breaks', l)
    // i=((460*j)+(40*l));
    i = 500*(l+1); // add dpi based rather than hardcode
    if(y>i){
      console.log('value of l, if loop', l)
      let bFlag = pageBreakNumberArray.includes(index)
      console.log('bFlag', bFlag)
      if(!bFlag ) {
        console.log('index', index)
        if(bFirstTimePageBreak){
          pageBreakNumberArray.push(index)
          allowDocPageBreak = true;
          bFirstTimePageBreak  =false;
          prevPageBreakIndex = index;
          wasPrevPageBreakFirst = true;
        }else {
          //for the first time page break 20,37 , skip 37
          if(wasPrevPageBreakFirst){
            wasPrevPageBreakFirst = false;
            allowDocPageBreak = false;
          }else if ((index-prevPageBreakIndex) == 1){
            allowDocPageBreak = true;
            pageBreakNumberArray.push(index)
          }
        }
      ...
      if(allowDocPageBreak){
        callstack.editorInfo.ace_doInsertPageBreak(pageBreakNumberArray);
        // setTimeout(() => {
        //   console.log("After timeout"); 
        //   l=$('iframe[name="ace_outer"]').contents().find('iframe').contents().find("#innerdocbody").children("div").find('.pageBreak').length;
        //   console.log('value of l, out side if', l)
        // }, 3000);
        // l = l + 1;
        l=$('iframe[name="ace_outer"]').contents().find('iframe').contents().find("#innerdocbody").children("div").find('.pageBreak').length;
        console.log('value of l, out side if', l)
    ...
    console.log(pageBreakNumberArray)  
  }
